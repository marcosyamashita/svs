...
$arq = fopen( "eleitores.txt", "r" ) or die( "ERRO, avise o webmaster, que falta um arquivo no servidor"); // abrindo o arquivo
...
for($c = 0; $c < sizeof($linha); $c++){
  ...
  $sql1 = "INSERT INTO `tb_login` (`id`,`nome`,`cpf`,`valida1`,`valida2`,`valida3`,`unidade`,`ativo`) VALUES ('".$coluna[0]."', '".$coluna[1]."', '".$coluna[2]."', '".$coluna[3]."', '".$coluna[4]."', '".$coluna[5]."', '".$coluna[6]."', '".$coluna[7]."')";
  ...
  $cont = "select COUNT(*) from tb_login where ativo = 0";
  $querycount = mysqli_query($link, $cont) or die(mysqli_error($link));
     while($row = mysqli_fetch_array($querycount)){
